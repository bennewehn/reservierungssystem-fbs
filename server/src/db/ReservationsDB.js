...
export async function getReservationsForPeriodDB(startTime, endTime) {
  const [result] = await pool.query(
    `SELECT user.firstName, user.lastName, reservation.startTime, reservation.endTime, reservation.rId, reservation.count, reservation.createdOn FROM reservation INNER JOIN user ON reservation.userId=user.userId ` + resPeriodCondition,
